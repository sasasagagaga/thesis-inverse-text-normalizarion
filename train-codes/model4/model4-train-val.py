# Обучаем модель №4 с подобранными ГП (по модели №1)
...
from ITN.data.data_loading import (
    generate_batch_for_classes_group_decoding,
    generate_batch_for_classes_group_decoding_UPC
)
...
from ITN.models.seq2seq_transformer_with_token_cls import (
    Seq2SeqTransformerWithClassLabelingGroupDecoderTrain,
    Seq2SeqTransformerWithClassLabelingGroupDecoderEval,
    seq2seq_transformer_classes_groups_translate,
    CLI_model4_train_UPC
)
...
def run_test(*, model_params, args):
    ...
    if not validation and train_utc or validation and val_utc:
        model_class = Seq2SeqTransformerWithClassLabelingGroupDecoderTrain
        generate_batch_clb = generate_batch_for_classes_group_decoding
        calc_loss_iter_clb = CLI_seq2seq_transformer
        translate_clb = seq2seq_transformer_translate
    else:
        model_class = Seq2SeqTransformerWithClassLabelingGroupDecoderEval
        if validation:
            generate_batch_clb = None
            calc_loss_iter_clb = None
        else:
            generate_batch_clb = generate_batch_for_classes_group_decoding_UPC
            calc_loss_iter_clb = CLI_model4_train_UPC
        translate_clb = seq2seq_transformer_classes_groups_translate
    ...
    if validation:
        ...
        trainer.train(
            ...
            compute_metrics_every_iters=200 if validation or train_utc else 10 ** 8
        )
...
def main():
    args = parse_args(model_number=4)
    print(args)
    ...
    if args.train_utc:
        params = configure_model_params(emb_size=512, num_encoder_layers=3, lr=3e-4)
    else:
        params = configure_model_params(emb_size=512, num_encoder_layers=3, lr=1e-4)
