# Обучаем модель №2 с подобранными ГП (по модели №1)
...
from ITN.data.data_loading import generate_batch_for_classes_labeling
...
from ITN.models.seq2seq_transformer_with_token_cls import (
    Seq2SeqTransformerWithTokensClassLabelingTrain,
    Seq2SeqTransformerWithTokensClassLabelingEval
)
...
def run_test(*, model_params, args):
    ...
    if not validation and train_utc or validation and val_utc:
        model_class = Seq2SeqTransformerWithTokensClassLabelingTrain
        generate_batch_clb = generate_batch_for_classes_labeling
    else:
        model_class = Seq2SeqTransformerWithTokensClassLabelingEval
...
def main():
    args = parse_args(model_number=2)
    print(args)
