# Обучаем модель №3 с подобранными ГП (по модели №1)
...
from ITN.data.data_loading import generate_batch_for_classes_range_labeling
...
from ITN.models.seq2seq_transformer_with_token_cls import (
    Seq2SeqTransformerWithTokensClassRangeLabelingTrain,
    Seq2SeqTransformerWithTokensClassRangeLabelingEval
)
...
def run_test(*, model_params, args):
    ...
    if not validation and train_utc or validation and val_utc:
        model_class = Seq2SeqTransformerWithTokensClassRangeLabelingTrain
        generate_batch_clb = generate_batch_for_classes_range_labeling
    else:
        model_class = Seq2SeqTransformerWithTokensClassRangeLabelingEval
...
def main():
    args = parse_args(model_number=3)
    print(args)
