# Обучаем модель №5 с подобранными ГП (по модели №1)
...
from ITN.data.data_loading import (
    generate_batch_for_classes_range_group_decoding,
    generate_batch_for_classes_range_group_decoding_UPC
)
...
from ITN.models.seq2seq_transformer_with_token_cls import (
    Seq2SeqTransformerWithClassRangeLabelingGroupDecoderTrain,
    Seq2SeqTransformerWithClassRangeLabelingGroupDecoderEval,
    seq2seq_transformer_classes_groups_translate,
    CLI_model5_train_UPC
)
...
def run_test(*, model_params, args):
    ...
    if not validation and train_utc or validation and val_utc:
        model_class = Seq2SeqTransformerWithClassRangeLabelingGroupDecoderTrain
        generate_batch_clb = generate_batch_for_classes_range_group_decoding
        calc_loss_iter_clb = CLI_seq2seq_transformer
        translate_clb = seq2seq_transformer_translate
    else:
        model_class = Seq2SeqTransformerWithClassRangeLabelingGroupDecoderEval
        if validation:
            generate_batch_clb = None
            calc_loss_iter_clb = None
        else:
            generate_batch_clb = generate_batch_for_classes_range_group_decoding_UPC
            calc_loss_iter_clb = CLI_model5_train_UPC
...
def main():
    args = parse_args(model_number=5)
